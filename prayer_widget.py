...
class PrayerTable(QWidget):
    def __init__(self):
        ...
        self.adhan = QMediaContent(
            QUrl.fromLocalFile(str(script_directory.joinpath("adhan.mp3").resolve()))
        )
        self.fajr_adhan = QMediaContent(
            QUrl.fromLocalFile(
                str(script_directory.joinpath("fajr_adhan.mp3").resolve())
            )
        )
